...
// The suite drives the browser through WXT's fakeBrowser, with a
...
function installBrowserMock(): Record<string, unknown> {
  ...
  local.set = (obj: Record<string, unknown>) => {
    Object.assign(store, obj);
    return Promise.resolve();
  };
  local.remove = (key: string | string[]) => {
...
function lastChallenge(): { nonce: string; context: string } {
  ...
  if (!frame) throw new Error("no frame was posted");

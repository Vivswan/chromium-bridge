...
| `page_eval` | **Critical** | anything the page can | **arbitrary JS** in the page | yes (can read tokens/cookies) | `scripting` (host) | **every-call** extension-window confirm showing the full code; result masked; kill-switch in options (Phase 8: Enclave Touch ID) |

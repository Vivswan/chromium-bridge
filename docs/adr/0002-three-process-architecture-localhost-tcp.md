...
- **ZCode** 会 spawn 一个进程作为 MCP server(stdio JSON-RPC)
...
1. **MCP server 进程**(ZCode spawn,长期):持有所有会话状态,监听 `127.0.0.1:0`(随机端口),把端口 + per-run secret 写到 0600 锁文件
...
- **不可行**:两个宿主(ZCode、Chrome)各自 spawn,进程不是父子关系,stdin/stdout 不共享。除非用 socket activation 之类的机制,但 Chrome 的 native messaging 不支持。

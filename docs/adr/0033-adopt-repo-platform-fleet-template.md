...
## Update (2026-09-11): the platform redesign

repo-platform retired Copier and template sync. A writer copies files from the platform's `files/` tree; every file's class is recorded in `.github/repo-platform-manifest.json` (`managed` replaced on every sync, `split` with a managed region only, `starter` written once and never overwritten, `link` a symlink). The managed `ci.yml` is now a thin skeleton: `checks.yml` plus repo-platform's `fleet-ci.yml`, `fleet-release.yml`, `fleet-release-publish.yml`, and `reusable-pages.yml`, all at `@build`, the platform's green-gated delivery branch. `release.yml`, `.github/.copier-answers.yml`, and `.github/CODE_OF_CONDUCT.md` are gone (the account defaults in Vivswan/.github cover the code of conduct); `CONTRIBUTING.md` and `.github/SECURITY.md` are fully repo-owned. The residual named above changes shape: third-party actions are SHA-pinned on both sides now, and the moving ref is `@build` alone.

...
browser-bridge 作为 MCP server,需要在 `initialize` 响应里声明自己说的协议版本,并按该版本实现。选错版本会导致 ZCode(客户端)握手失败或行为异常。
...
- **问题**:调研时**没有发布版客户端**(包括 ZCode/Claude-compatible)用这个 draft
- **排除**:用 draft 会导致与所有实际客户端不兼容

### 方案 B:用更老的 `2024-11-05`
- **问题**:旧版本的字段约定和能力模型与当前客户端实现有偏差
- **排除**:ZCode 实现的是 2025-06-18,用旧版本可能错过新约定
...
### 正面
- **与 ZCode 兼容**:ZCode 实现的就是这个版本,握手能通过
- **稳定**:协议版本锁定,不随 draft 漂移
- **最小实现**:只实现必需消息,代码量小,易审计

### 负面
- **未来要跟进**:MCP 若发布新稳定版且 ZCode 升级,browser-bridge 可能需要更新协议版本号 + 适配新约定

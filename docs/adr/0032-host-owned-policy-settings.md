...
- Extends: [ADR-0021](0021-enrollment-ceremony.md) (the Secure Enclave
  ...
  becomes the policy-editing surface), [ADR-0031](0031-touch-id-confirmations-and-presence-grants.md)
...
1. Policy moves host-side. The capability grants (`cdpMode`,
   ...
   `requireEnrollment`, `hostReverifyMs`, `confirmGraceMs`), the per-tool
...
the nine confirmation-policy fields, `disabledTools`, and the two
...
not to one browser profile, and their editing surface is the desktop app.
...
and then strict-parses those same bytes. The document carries a monotonic
`revision` inside the signed bytes, a u64 constrained to the JS-safe
integer range (the same posture as `BridgeReq::id`), so both parsers read
the same number.
...
than the stored one. Replaying the current baseline unchanged is
...
no matter what signature the stale bytes carry. The anchor for all of this
...
signature), changes nothing: the extension keeps enforcing the stored
...
`requireEnrollment` true. Where that state can occur is bounded by the
...
same-user process that edits the baseline bytes breaks the signature; one
...
which is what the desktop app calls in-process, the same pattern as
`kill::release` and `allowlist::pair_client_with_presence`. The CLI gains a
read surface (`chromium-bridge policy`, and `doctor` reports the policy
state and signature validity); policy editing is app-first and the CLI gets
no write path in this record, because a second grant surface should be added
...
extension UI drops the release control while keeping engage. The
...
immediately, since they only remove capability. `uiLanguage` is imported
...
- Why a value ratchet instead of only a revision counter. Revisions alone
  ...
  protect, and it makes the security statement local: this extension never
  gets more permissive without a fresh signature, full stop.
...
- Policy has one owner, one editing surface, one enforcement contract, and
  the grant direction is hardware-gated end to end: the tap that ADR-0031
...
- The #32 cold-start window now also covers the policy ratchet, and
  ...
  fail-closed DoS.
- Users who never open the desktop app keep defaults only: previously
  relaxed settings (an enabled `page_upload`, a disabled confirmation) stop
  working after cutover until re-granted through the app. The import flow
...
gets backwards), the JS-safe revision bound, store round-trips and tamper
...
any sheet), audits every transition. CLI: `policy` read-out, `doctor` rows.
Tests: cargo units for dispatch refusal and the kill_release refusal; e2e
...
signature, forged restriction accepted, forged relaxation refused, failed

...
The machinery is the managed `.github/workflows/ci.yml`, downstream of the all-green gate, so a release can only ever be cut from a green `main`, and the packaging jobs run in that same CI run. Its jobs, in order: `release` calls repo-platform's `fleet-release.yml` (release-please cuts the draft); `update-release` calls this repository's hook, only when release-please reports `release_created`; `publish-release` calls repo-platform's `fleet-release-publish.yml` (attest, then publish); `pages` deploys the site after the publish, in the same run. `update-release-pr` calls the repo-owned `update-release-pr.yml` hook whenever release-please creates or refreshes the release PR. The `REPO_PLATFORM_TOKEN` repository secret (a PAT) matters for the release PR: release-please creates it with that token so the PR gets CI runs (with only `GITHUB_TOKEN` the release PR needs a close/reopen to run its checks). Configure the secret before the first release.

...
3. The managed `publish-release` stage waits for every hook job, attests build provenance for every asset on the draft into a single `attestation.jsonl` asset, and flips the draft live. Publishing is structurally last: the hook cannot reorder or skip it.

If the pipeline dies after the draft exists, a rerun cannot recreate it (release-please sees the forced tag); re-run the failed jobs, or finish by hand with `gh release upload <tag> <assets> --clobber` and `gh release edit <tag> --draft=false` (a hand-published release carries no `attestation.jsonl`).

The machinery is the managed `.github/workflows/release.yml` pipeline - release-please cuts the draft, the repo-owned `.github/workflows/update-release.yml` hook mutates it, the managed publish stage attests and publishes - which the managed ci.yml runs downstream of the all-green gate, so a release can only ever be cut from a green `main`, and the packaging jobs run in that same CI run (the hook is only called when release-please reports `release_created`). The `REPO_PLATFORM_TOKEN` repository secret (a PAT) matters twice: release-please creates the release PR with it so that PR gets CI runs (with only `GITHUB_TOKEN` the release PR needs a close/reopen to run its checks), and `publish-release` publishes with it so the `release: published` event fires the Pages production deploy (events from `GITHUB_TOKEN` trigger no workflows - without the secret, dispatch pages.yml by hand after a release). Configure the secret before the first release.
...
5. `gh release upload` attaches the assets to the draft release; the managed `publish-release` stage then attests every asset on the draft into the release-level `attestation.jsonl` and publishes it once every hook job is done.

Users therefore **do not need a Rust/bun toolchain** to install: registration is the binary's own `chromium-bridge doctor --fix` (or the desktop app), see [quickstart.md](./quickstart.md). Third-party Actions in the repo-owned workflows are pinned to commit SHAs; the managed release.yml floats its refs by fleet policy, a recorded residual risk in [ADR-0033](./adr/0033-adopt-repo-platform-fleet-template.md).

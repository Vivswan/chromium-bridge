...
3. The managed `publish-release` stage waits for every hook job, attests build provenance for every asset on the draft into a single `attestation.json` asset, and flips the draft live. Publishing is structurally last: the hook cannot reorder or skip it.
...
5. `gh release upload` attaches the assets to the draft release; the managed `publish-release` stage then attests every asset on the draft into the release-level `attestation.json` and publishes it once every hook job is done.

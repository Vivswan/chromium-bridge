...
扩展本身以 **load-unpacked** 方式从 **`extension/dist/`**(esbuild 构建产物,由 `src/*.ts` 打包 + 静态资源拷贝而来)加载,`install.sh` 会先构建再加载。dist/ 不入库,克隆后须先 `npm run build`(或 `make ext-build`)。详见 [ADR-0012](./adr/0012-typescript-esbuild-extension-build.md)。

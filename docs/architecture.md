...
| `tools.rs` | 15 个工具的 schema 定义 + `HANDLERS` 注册表(`{name, build_payload}` 纯函数)→ dispatch 到 session.call。详见 [RFC-0002](./rfc/0002-tool-registry.md) |
| `session.rs` | 连接管理 + 请求/响应按 id 配对(mpsc channel per id)+ 每连接 generation id(修 writer-clobber 竞态、断开即 `Disconnected` 排空)+ 120s 超时。详见 [RFC-0001](./rfc/0001-connection-state-machine.md) |
...
- **加新工具**:在 `tools.rs` 加 schema 定义 + 一条 `HANDLERS` 记录(`build_payload` 纯函数),扩展 background/content 加对应 op 处理(见 [RFC-0002](./rfc/0002-tool-registry.md))
...
以下两篇 RFC 已落地(见各自的 "Implementation notes");它们细化了连接与工具分发:

- **[RFC-0002:工具注册表 / Command 模式](./rfc/0002-tool-registry.md)**(Implemented)——
  `src/tools.rs` 已从一个大 `match` 换成 **`HANDLERS` 注册表**:每个工具是一条
  `Handler { name, build_payload }` 记录,`build_payload` 是把 MCP 参数塑形为 op 参数的**纯函数**,
  `dispatch` 按 `name` 查表转发。契约一致性由 `registry_covers_catalogue`(覆盖
  [tools.json](../contracts/tools.json))与 `build_payload_shapes` 两个测试守卫。
  落地采用了比提案更轻量的函数表形态而非 `ToolHandler` trait;能力门(§11.2)随握手一并留待后续。
- **[RFC-0001:显式化连接状态机](./rfc/0001-connection-state-machine.md)**(Implemented,首阶段)——
  §5.2 的连接/重连已引入 **每连接 generation id**(存于 `src/session.rs`,与 writer 同放):
  reader 仅在 generation 与当前一致时才清空 writer(修掉重连时的 writer-clobber 竞态),
  in-flight 请求按 generation 快速排空、断开即以 `CallError::Disconnected` 结束而不等 120s 超时。
  完整五态枚举与 §11.2 的版本/能力握手(`PROTOCOL_MISMATCH` 快速失败)属后续阶段。

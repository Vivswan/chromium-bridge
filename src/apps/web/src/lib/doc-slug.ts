...
// The docs the site ships from outside docs/. This is an allowlist on
// purpose: the root glob in docs.ts picks up whatever sits in the checkout,
// including untracked scratch notes, so an unrecognized root file must fail
// closed (not rendered) instead of publishing by accident. Rendering a new
...
export function repoPathToSlug(rel: string): string | undefined {
  ...
  if (!rel.startsWith("docs/")) {
    ...
    // .github/ is GitHub's community-health layout, not a site section, so
    // the security policy keeps the /docs/SECURITY/ route it had when the
    // file lived at the repo root.
    return rel
      .replace(/^\.github\//, "")
      .replace(/\.md$/, "")

...
export async function tabList() {
  ...
  // groupId is -1 (browser.tabGroups.TAB_GROUP_ID_NONE) for ungrouped tabs;
...
// tab via browser.tabs, so they need no content script. Every one gates on the
...
export async function tabOpen(url: string) {
  await ensureAllowed(url);
  const t = await browser.tabs.create({ url });
...
async function addToWorkspaceGroup(
  ...
  try {
    const groups = await browser.tabGroups.query(windowId != null ? { windowId } : {});

...
const identity = JSON.parse(readFileSync(join(root, "contracts/identity.json"), "utf8")) as {
  ...
  extensionManifestKey?: unknown;
};
if (
  typeof identity.extensionManifestKey !== "string" ||
  identity.extensionManifestKey.length === 0
) {
  throw new Error("gen-ops: contracts/identity.json has no extensionManifestKey");
}
...
const hex = createHash("sha256")
  .update(Buffer.from(identity.extensionManifestKey, "base64"))
  .digest("hex")
  .slice(0, 32);
...
// contracts/identity.json's \`extensionManifestKey\` (Chrome's own id
// derivation), so it cannot drift from the generated manifest; NATIVE_HOST_ID
// comes from the same contract. scripts/check-extension-id.ts verifies the
// installers and the Rust host against the same sources.
